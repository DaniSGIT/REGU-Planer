...
function normalizeImportedPriceRows(rows) {
  ...
  rows.forEach((raw) => {
    ...
    for (let i = 0; i < values.length; i += 1) {
      const materialRaw = values[i];
      const material = cleanImportedPriceMaterial(materialRaw);

      if (!isValidImportedPriceMaterial(material)) continue;

      let foundPrice = 0;

      // Preis steht je nach Excel direkt daneben oder 1–2 Spalten weiter
      for (let offset = 1; offset <= 3; offset += 1) {
        const priceCandidate = values[i + offset];
        const priceTo = parseImportedPriceNumber(priceCandidate);
      ...
      // Schutz gegen Überschriften / Kategorien mit kleinen Zahlen wie "Messing ... 32"
      if (foundPrice < 50) continue;
