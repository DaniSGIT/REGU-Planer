...
function bindHofbook() {
  $("#hofbookPrevDayBtn")?.addEventListener("click", () => shiftHofbookDay(-1));
  $("#hofbookTodayBtn")?.addEventListener("click", () => {
    state.settings.hofbookDate = dateKey(new Date());
    saveState();
    renderHofbook();
  });
  $("#hofbookNextDayBtn")?.addEventListener("click", () => shiftHofbookDay(1));
...
function openHofbookEntryModal() {
  ...
  if (dateInput) dateInput.value = getHofbookDate();
...
function buildHofbookHintPill(entry) {
  return `
    <div class="hofbook-hint-pill ${entry.done ? "done" : ""}">
      <span>Hinweis</span>
...
function buildHofbookOrderLine(entry) {
  return `
    <div class="hofbook-paper-line ${entry.done ? "done" : ""}">
      <button
        class="hofbook-check"
        type="button"
      ...
      <div class="hofbook-line-content">
