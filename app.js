(function () {
  ...
  function checkForUpdate() {
    ...
    if (savedVersion && savedVersion !== APP_VERSION) {
      const updateNow = confirm(
        "Eine neue Version ist verfügbar.\n\nVor dem Aktualisieren wird automatisch ein Backup heruntergeladen.\n\nJetzt aktualisieren?"
    );
  ...
  function renderDashboardCommandCards() {
    ...
    const cards = [
      {
        ...
        meta: priceStatus.needsAttention && priceStatus.officeLabel
          ? `${newPricesMeta} · ${priceStatus.officeLabel}`
          : newPricesMeta,
        icon: "briefcase",
        tab: "dashboard",
    ...
    target.innerHTML = cards
      .map((card) => `
        <button class="command-card" type="button" data-command-tab="${card.tab}" data-command-title="${escapeHtmlAttr(card.title)}" data-command-detail="${escapeHtmlAttr(card.detail)}" data-command-drawer="${card.drawerType || "default"}">
  ...
  async function loadStateFromSupabase() {
    try {
      ...
      if (data?.data) {
        ...
        saveState(); // lokale Kopie aktualisieren
      }
    } catch (err) {
