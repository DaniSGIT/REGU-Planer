(function () {
  ...
  "Hofbuch-Ansicht: Post-Its können jetzt per Drag & Drop verschoben werden",
  "Personal-Nummern können jetzt in der Steuerberater-Ansicht angezeigt werden",
];
...
function buildHofbookBookPage(dayKey, pageSide = "left") {
  ...
  const pageStatus = getHofbookPageStatus(dayKey);
  const dateBadges = buildHofbookBookDateBadges(dayKey);
  const emptyLineCount = Math.max(8, 16 - entries.length);

  return `
 <section class="hofbook-book-page ${pageSide} ${isToday ? "today" : ""} ${pageStatus.type ? `hofbook-page-${pageStatus.type}` : ""}" data-hofbook-page-date="${escapeHtmlAttr(dayKey)}">
      <header class="hofbook-book-page-head">
        <div>
          ...
          ${
            pageStatus.label ?
               `<span class="hofbook-page-status-badge ${escapeHtmlAttr(pageStatus.type)}">${escapeHtml(pageStatus.label)}</span>`
              : ""
          }
        </div>
      ...
      ${
        dateBadges.trim() ?
           `<div class="hofbook-book-badge-row">${dateBadges}</div>`
          : ""
      }
