...
let currentUserProfile = null;
  ...
  const defaultData = {
    ...
    employees: [
      { id: uid(), name: "Yesim Kröll", personnelNumber: "", department: "Buero", phone: "", entryDate: "", birthday: "", active: true, notes: "", vacationAllowance: 24, vacationCarryoverByYear: {} },
      { id: uid(), name: "Daniela Leins", personnelNumber: "", department: "Buero", phone: "", entryDate: "", birthday: "", active: true, notes: "", vacationAllowance: 30, vacationCarryoverByYear: {} },
      { id: uid(), name: "Christian Hansen", personnelNumber: "", department: "Lager", phone: "", entryDate: "", birthday: "", active: true, notes: "", vacationAllowance: 30, vacationCarryoverByYear: {} },
      { id: uid(), name: "Nico Kastelberger", personnelNumber: "", department: "Lager", phone: "", entryDate: "", birthday: "", active: true, notes: "", vacationAllowance: 30, vacationCarryoverByYear: {} },
      { id: uid(), name: "Andreas Rudolph", personnelNumber: "", department: "Lager", phone: "", entryDate: "", birthday: "", active: true, notes: "", vacationAllowance: 30, vacationCarryoverByYear: {} },
      { id: uid(), name: "Timon Guttenberger", personnelNumber: "", department: "Lager", phone: "", entryDate: "", birthday: "", active: true, notes: "", vacationAllowance: 30, vacationCarryoverByYear: {} }
    ],
    attendance: {},
...
containers: []
  ...
  let hofbookPostItDrag = null;
  let containerVisibleRows = [];
  let containerListMode = "recent";
  let employeeAdminSearchTerm = "";
  let attendanceDetailsOpenEmployeeId = "";
  let priceListSearchTerm = "";
  let vacationPlanActiveTab = "employees";
  let feedbackItems = [];
  const FEEDBACK_MANAGER_ROLES =
  new Set(["admin"]);
  let feedbackEditId = "";
let feedbackActiveFilter = "all";
let feedbackSearchTerm = "";
let lastNonFeedbackTabId = "hofbuch";
...
let reguSorterState = null;
  ...
  async function init() {
  localStorage.removeItem("regu_theme_mode");
...
function safeStartupStep(label, fn) {
  try {
    ...
    console.error(`Startschritt übersprungen: ${label}`, error);
  }
}
...
async function startAppAfterLogin() {
  ...
  safeStartupStep("bindTabs", bindTabs);
safeStartupStep("bindTop", bindTop);
safeStartupStep("bindAppShellMenu", bindAppShellMenu);
safeStartupStep("bindWorkStartScreen", bindWorkStartScreen);
safeStartupStep("bindGlobalUi", bindGlobalUi);
safeStartupStep("bindOffice", bindOffice);
safeStartupStep("bindContainers", bindContainers);
safeStartupStep("bindHofbook", bindHofbook);
safeStartupStep("bindAttendance", bindAttendance);
safeStartupStep("bindHoursBillingSubtabs", bindHoursBillingSubtabs);
safeStartupStep("bindVacationPlanner", bindVacationPlanner);
safeStartupStep("bindStats", bindStats);
safeStartupStep("bindDashboard", bindDashboard);
safeStartupStep("bindExports", bindExports);
safeStartupStep("bindPriceList", bindPriceList);
safeStartupStep("bindSettings", bindSettings);
safeStartupStep("bindFeedback", bindFeedback);
safeStartupStep("bindWasteCalendar", bindWasteCalendar);
...
function applyRoleUi() {
 ...
 button.style.display = allowed ? "" : "none";
    ...
    .forEach((group) => {
      const hasVisibleTab = Array
        .from(group.querySelectorAll(".tab"))
        .some((button) =>
...
const sickInfo = getBillingSickDays(
  employeeId,
  rangeDays,
  holidayMap
);
  ...
  if (!isEmployeeEmployedOnDate(employee, day)) return;
...
function exportTaxHoursPdf() {
 ...
 const rows = state.employees
  .filter((employee) =>
    isEmployeeRelevantForPeriod(employee, view)
  )
  .map((employee) => {
  ...
  margin: 0 0 6mm;
      ...
      <body>
        ...
        <table>
          <thead>
            <tr>
              <th class="personnel-number">Nr.</th>
              <th>Mitarbeiter</th>
          ...
          <tbody>
            ${allRows.map((row) => `
              <tr>
                <td class="personnel-number">${escapeHtml(row.personnelNumber || "")}</td>
...
$("#taxAdvisorAddressInput")?.addEventListener("input", (event) => {
  ...
  if (
    ...
    lastNonFeedbackTabId = tabId;
  }

  if (
    tabId !== "nixzutun" &&
    typeof hideNixZuTunMenu === "function"
  ) {
    hideNixZuTunMenu();
  }
...
function updateReguAppbarTitle(tabId = "hofbuch") {
  const title = $("#reguAppbarPageTitle");
  if (!title) return;

  const labels = {
    hofbuch: "Hofbuch",
    anwesenheit: "Tageserfassung",
    preislisten: "Preislisten",
    container: "Container",
    stundenabrechnung: "Stundenabrechnung",
    buero: "Büroplan",
    notizen: "Notizen & Termine",
    fahrzeuge: "Fahrzeuge",
    urlaubsplan: "Urlaubsplanung",
    statistik: "Statistik",
  ...
  title.textContent =
    labels[tabId] || "REGU Planer";
}

function closeSidebarSubmenus() {
  document
    .querySelectorAll(".sidebar-menu-group.open")
    .forEach((group) => {
...
let sidebarSubmenuCloseTimer = null;

function usesSidebarHoverNavigation() {
  return window.matchMedia(
    "(hover: hover) and (min-width: 761px)"
  ).matches;
}

function openSidebarSubmenu(group) {
  if (!group) return;

  window.clearTimeout(
    sidebarSubmenuCloseTimer
  );
...
function closeSidebarSubmenuDelayed(
  group
) {
  window.clearTimeout(
    sidebarSubmenuCloseTimer
...
function bindAppShellMenu() {
  ...
  toggle?.addEventListener("click", () => {
    ...
    const isOpen =
      shell.classList.contains("menu-open");
  ...
  .forEach((group) => {
    ...
    group.addEventListener(
      ...
      () => {
        if (
          !usesSidebarHoverNavigation()
        ) {
          return;
        }

        openSidebarSubmenu(group);
      }
    );
    ...
    button?.addEventListener(
      ...
      (event) => {
        ...
        /*
         * Auf einem PC übernimmt die Maus.
         * Der Klick muss dort nichts festpinnen.
         */
        if (
          usesSidebarHoverNavigation()
        ) {
        ...
        /*
         * Auf Touchgeräten bleibt die bisherige
         * Bedienung per Klick erhalten.
         */
        const shouldOpen =
  ...
  document?.addEventListener("click", (event) => {
    ...
    const clickedToggle =
      event.target.closest("#appMenuToggle");
...
function setAppMenuOpen(open) {
  ...
  if (open) {
  const sidebarNav =
    shell.querySelector(".sidebar-nav");
  ...
  root.innerHTML = `
    <div class="regu-sorter-shell">
      ...
      <div class="regu-sorter-bins">
        ...
        <button type="button" data-regu-bin="rest">??? <strong>Rest</strong><small>Restmüll, Mischzeug</small></button>
        <button type="button" data-regu-bin="problem">?? <strong>Problemstoff</strong><small>Batterie, Ölfilter</small></button>
      </div>
...
function getReguSorterItems() {
  return [
    { emoji: "??", name: "Alu-Felge", type: "metall" },
    ...
    { emoji: "???", name: "Ölfilter", type: "problem" },
    { emoji: "??", name: "Unbekannte Flüssigkeit", type: "problem" }
  ];
}
  ...
  if (root) {
    root.innerHTML = `
      <div class="regu-sorter-finish">
        <div class="regu-sorter-finish-icon">??</div>
  ...
  if (root) {
    root.innerHTML = `
      <div class="regu-quiz-finish">
        <div class="regu-quiz-finish-icon">??</div>
...
function bindHofbook() {
  ...
  const openEntryButton =
  $("#hofbookOpenEntryModalBtn");
...
if (
  ...
  postItButton.className =
    "ghost hofbook-postit-add-btn";

  postItButton.textContent =
    "Post-it";

  openEntryButton.insertAdjacentElement(
    "afterend",
    postItButton
  );
}
...
if (postItButton) {
  postItButton.onclick =
    addHofbookPostIt;
}

$("#hofbookPaper")?.addEventListener(
  "pointerdown",
  startHofbookPostItDrag
);

document?.addEventListener(
  "pointermove",
  moveHofbookPostItDrag
);

document?.addEventListener(
  "pointerup",
  endHofbookPostItDrag
);
...
document?.addEventListener(
  ...
  const movedPostIts =
    carryOpenHofbookPostItsToToday();

  state.settings.hofbookDate =
    dateKey(new Date());
  ...
  if (movedPostIts > 0) {
    showToast(
      movedPostIts === 1
        ? "1 offenes Post-it wurde auf heute übernommen."
        : `${movedPostIts} offene Post-its wurden auf heute übernommen.`,
      "success"
    );
  }
});
  ...
  document?.addEventListener("click", (event) => {
    const deleteBossBtn = event.target.closest("[data-hofbook-delete-boss-absence]");
    if (deleteBossBtn) {
      event.preventDefault();
      event.stopPropagation();

      const code = deleteBossBtn.dataset.hofbookDeleteBossAbsence || "";
      const dayKey = deleteBossBtn.dataset.hofbookDeleteBossAbsenceDate || getHofbookDate();

      state.hofbookEntries = (state.hofbookEntries || []).filter((entry) => {
        if (entry.date !== dayKey) return true;
        return !getBossAbsenceCodeFromText(entry.text || "").includes(code);
      });
    ...
    const manualBadgeDelete = event.target.closest("[data-hofbook-badge-delete], [data-hofbook-delete-badge]");
    if (manualBadgeDelete) {
      event.preventDefault();
      event.stopPropagation();
      deleteHofbookManualBadge(manualBadgeDelete.dataset.hofbookBadgeDelete || manualBadgeDelete.dataset.hofbookDeleteBadge);
      return;
    }
    ...
    if (event.target.closest("[data-hofbook-open-events], [data-open-events]")) {
      activateTab("notizen");
      return;
    }
  ...
  document?.addEventListener("click", (event) => {
    ...
    const clickedInsideNotesPopover =
  event.target.closest(
    ".hofbook-note-popover-wrap, .hofbook-quick-notes-wrap"
  );

const clickedNotesBadge =
  event.target.closest(
    "[data-hofbook-open-notes], [data-hofbook-quick-action='notes']"
  );

if (
  clickedInsideNotesPopover ||
  clickedNotesBadge
) {
  return;
}
...
renderHofbookQuickDashboard();
  ...
  $("#hofbookSearchToggleBtn")?.addEventListener(
  "click",
  () => {
    const panel =
      $("#hofbookSearchPanel");
...
$("#hofbookQuickDashboard")?.addEventListener("click", (event) => {
   const closeNotesPreview = event.target.closest(
    "[data-hofbook-quick-notes-close]"
  );

  if (closeNotesPreview) {
    hofbookNotesExpanded = false;
    renderHofbookQuickDashboard();
    return;
  }
  const button = event.target.closest(
    "[data-hofbook-quick-action]"
  );

  if (!button) return;

  const action =
    button.dataset.hofbookQuickAction || "";

  if (action === "new-prices") {
    triggerNewPricesAction();
    renderHofbookQuickDashboard();
    return;
  }

  if (action === "yesim-hours") {
    openDetailDrawer(
      buildYesimHoursDrawerHtml()
    );
    return;
  }
  ...
  if (action === "upcoming") {
    openDetailDrawer(
      buildHofbookQuickUpcomingHtml()
    );

    bindHofbookQuickDrawerLinks();
    return;
  }

    if (action === "notes") {
    const { leftKey, rightKey } =
      getHofbookBookSpreadKeys(
        getHofbookDate()
      );
    ...
    const hasNotes = [
      ...
      (dayKey) =>
        getHofbookNotesForDay(dayKey)
          .length > 0
    );

    if (!hasNotes) {
      showToast(
        "Auf dieser Doppelseite gibt es keine Notizen.",
        "success"
      );
      return;
    }

    hofbookNotesExpanded =
      !hofbookNotesExpanded;
  ...
  $("#hofbookCockpit")?.addEventListener("click", (event) => {
  const actionButton = event.target.closest(
    "[data-hofbook-cockpit-action]"
  );

  if (!actionButton) return;

  const action =
    actionButton.dataset.hofbookCockpitAction || "";

  if (action === "postits") {
    const shelfTrack =
      $("#hofbookPostItShelf .hofbook-postit-shelf-track");

    if (shelfTrack) {
      shelfTrack.scrollIntoView({
        behavior: "smooth",
        block: "center"
      });
    }
  ...
  if (action === "notes") {
    const { leftKey, rightKey } =
      getHofbookBookSpreadKeys(getHofbookDate());

    const noteDay = [leftKey, rightKey].find(
      (dayKey) =>
        getHofbookNotesForDay(dayKey).length > 0
    );
    ...
    state.settings.hofbookDate = noteDay;
    hofbookNotesExpanded = true;
...
$("#hofbookPostItShelf")?.addEventListener(
  "click",
  (event) => {
    const editButton = event.target.closest(
      "[data-hofbook-postit-edit]"
    );

    if (editButton) {
      editHofbookPostIt(
        editButton.dataset.hofbookPostitEdit
      );
    ...
    const scheduleButton = event.target.closest(
      "[data-hofbook-postit-schedule]"
    );

    if (scheduleButton) {
      openHofbookMoveModal(
        scheduleButton.dataset.hofbookPostitSchedule
      );
    ...
    const deleteButton = event.target.closest(
      "[data-hofbook-postit-delete]"
    );

    if (deleteButton) {
      deleteHofbookPostIt(
        deleteButton.dataset.hofbookPostitDelete
      );
    }
  }
);
...
const postItSaveButton =
  event.target.closest(
    "[data-hofbook-postit-save]"
  );
...
if (postItSaveButton) {
  ...
  saveHofbookPostItDraft(
    postItSaveButton.dataset
      .hofbookPostitSave
  );
...
const postItCancelButton =
  event.target.closest(
    "[data-hofbook-postit-cancel]"
  );

if (postItCancelButton) {
  event.preventDefault();
  event.stopPropagation();

  cancelHofbookPostItDraft(
    postItCancelButton.dataset
      .hofbookPostitCancel
  );
...
const postItScheduleButton =
  event.target.closest(
    "[data-hofbook-postit-schedule]"
  );

if (postItScheduleButton) {
  event.preventDefault();
  event.stopPropagation();

  openHofbookMoveModal(
    postItScheduleButton.dataset
      .hofbookPostitSchedule
  );

  return;
}

const postItEditButton =
  event.target.closest(
    "[data-hofbook-postit-edit]"
  );
...
if (event.target.closest(".hofbook-postit")) {
  return;
}
...
if (event.target.closest(".hofbook-inline-entry")) {
    ...
    const toPostItButton = event.target.closest(
  "[data-hofbook-to-postit]"
);

if (toPostItButton) {
  moveHofbookEntryToPostIt(
    toPostItButton.dataset.hofbookToPostit
  );
  ...
  return;
    ...
    if (
      ...
      !event.target.closest(".hofbook-book-badge-row") &&
...
function openHofbookEntryModal(editId = "") {
  ...
  const entry = selectedHofbookEditId
    ? (state.hofbookEntries || []).find(
        (item) => item.id === selectedHofbookEditId
      ) || null
    : null;
  ...
  if (typeInput) {
    typeInput.value =
      entry && ["hint", "container", "badge"].includes(entry.type)
        ? entry.type
        : "container";
  }
  ...
  if (dateInput) {
    dateInput.value = entry?.date || getHofbookDate();
  }
  ...
  return (Array.isArray(entries) ? entries : [])
    ...
    .map((entry) => {
      ...
      if (type === "postit") {
        ...
        normalized.spreadKey = String(
          entry.spreadKey ||
          entry.date ||
          getHofbookBookSpreadStartKey(
            dateKey(new Date())
          )
        );

        normalized.x = Number.isFinite(rawX)
          ? Math.min(90, Math.max(0, rawX))
          : 8;

        normalized.y = Number.isFinite(rawY)
          ? Math.min(82, Math.max(0, rawY))
          : 12;
      }
...
function getHofbookVacationItems(dayKey) {
  return (state.employees || [])
    ...
    .map((employee) => {
      const entry = getAttendanceEntry(employee.id, dayKey);
...
function getHofbookSickItems(dayKey) {
  return (state.employees || [])
    .filter((employee) => employee.active !== false)
    .map((employee) => {
      const entry = getAttendanceEntry(employee.id, dayKey);
...
function getHofbookManualBadges(dayKey) {
  return (state.hofbookEntries || [])
    ...
    .filter((entry) => !isHofbookBossAbsenceEntry(entry))
    .sort((a, b) =>
...
function buildHofbookBookManualBadge(entry) {
  return `
    <span class="hofbook-book-badge manual" title="${escapeHtmlAttr(entry.text || "")}">
      ${escapeHtml(entry.text || "Badge")}
      <button
        class="hofbook-book-badge-delete"
        type="button"
        title="Badge löschen"
        data-hofbook-delete-badge="${escapeHtmlAttr(entry.id)}">&times;</button>
    </span>
  `;
}

function getAttendanceRequiredEmployees(dayKey) {
  const office =
    state.officePlan?.[dayKey] || {};

  const day =
    parseDateKey(dayKey);

  const plannedOfficeEmployeeIds =
    new Set(
      [
        office.primaryEmployeeId,
        office.secondaryEmployeeId
  ...
  return (state.employees || [])
    .filter((employee) =>
      isEmployeeAttendanceEditableOnDate(
        employee,
        day
      )
    )
    .filter((employee) => {
      ...
      /*
       * Lager-Mitarbeiter müssen vollständig
       * erfasst werden.
       *
       * Büro-Mitarbeiter nur dann, wenn sie
       * an diesem Tag im Büroplan stehen.
       */
      return (
        !isOfficeEmployee ||
        plannedOfficeEmployeeIds.has(
          employee.id
        )
      );
    });
...
function getAttendanceCompletionInfo(
  dayKey
) {
  const day =
    parseDateKey(dayKey);
  ...
  const specialDay =
    getSpecialOfficeDay(dayKey);

  const useSaturdayRule =
    isSaturday &&
    !holidayMap[dayKey] &&
    specialDay.mode !== "closed";

  /*
   * Samstag:
   * Eine anwesende Büro-Person
   * und eine anwesende Lager-Person reichen.
   */
  if (useSaturdayRule) {
    const officePlan =
      state.officePlan?.[dayKey] || {};

    const plannedOfficeIds =
      new Set(
        [
          officePlan.primaryEmployeeId,
          officePlan.secondaryEmployeeId
        ].filter(Boolean)
      );
    ...
    const plannedOfficeEmployees =
      officeEmployees.filter(
        (employee) =>
          plannedOfficeIds.has(
            employee.id
          )
      );

    /*
     * Steht jemand im Büroplan,
     * muss eine dieser Personen anwesend sein.
     *
     * Ist niemand geplant, reicht irgendeine
     * aktive Büro-Person.
     */
    const eligibleOfficeEmployees =
      plannedOfficeEmployees.length
        ? plannedOfficeEmployees
        : officeEmployees;
    ...
    const officePresent =
      eligibleOfficeEmployees.some(
        (employee) => {
          const entry =
            getRawAttendanceEntry(
              employee.id,
              dayKey
            );

          return entry?.status === "A";
        }
      );

    const warehousePresent =
      warehouseEmployees.some(
        (employee) => {
          const entry =
            getRawAttendanceEntry(
              employee.id,
    ...
    return {
      ...
      completedCount:
        Number(officePresent) +
        Number(warehousePresent),
  ...
  const requiredEmployees =
    getAttendanceRequiredEmployees(
      dayKey
    );

  const completedEmployees =
    requiredEmployees.filter(
      (employee) => {
        const entry =
          getRawAttendanceEntry(
            employee.id,
  ...
  return {
    ...
    isSaturdayRule: false
  };
}

function isAttendanceCompleteForDay(
  dayKey
) {
  return getAttendanceCompletionInfo(
    dayKey
  ).isComplete;
}

function getMissingAttendanceNames(
  dayKey
) {
  return getAttendanceCompletionInfo(
    dayKey
  ...
  if (complete) {
    target.innerHTML = `
      <button type="button" class="hofbook-attendance-notice-btn done" data-open-attendance>
        ...
        <small>Alle erforderlichen Mitarbeiter haben einen Status.</small>
      </button>
    `;
    return;
  }
...
function buildHofbookPostItShelfCard(entry) {
  const createdLabel = entry.createdAt
    ? formatDate(new Date(entry.createdAt))
    : "";

  return `
    <article class="hofbook-postit-shelf-card">
      <div class="hofbook-postit-shelf-text">
        ${escapeHtml(entry.text || "").replace(/\n/g, "<br>")}
      </div>

      <div class="hofbook-postit-shelf-meta">
        ${
          createdLabel
            ? `<small>Seit ${escapeHtml(createdLabel)}</small>`
            : "<small>Offenes Post-it</small>"
        }

        <div class="hofbook-postit-shelf-actions">
          <button
            type="button"
            title="Text bearbeiten"
            data-hofbook-postit-edit="${escapeHtmlAttr(entry.id)}">
            ✎
          </button>

          <button
            type="button"
            title="Datum eintragen und ins Hofbuch übernehmen"
            data-hofbook-postit-schedule="${escapeHtmlAttr(entry.id)}">
            📅
          </button>

          <button
            type="button"
            title="Post-it löschen"
            data-hofbook-postit-delete="${escapeHtmlAttr(entry.id)}">
            ×
          </button>
        </div>
      </div>
...
function renderHofbookCockpit() {
  const target = $("#hofbookCockpit");
  const shelf = $("#hofbookPostItShelf");
  ...
  const spreadKeys = [leftKey, rightKey];

  const postIts =
    getHofbookPostItsForSpread(leftKey);

  const openEntries = spreadKeys
    .flatMap((dayKey) =>
      getHofbookEntriesForBookPage(dayKey)
    )
    .filter((entry) => !entry.done);

  const notes = spreadKeys
    .flatMap((dayKey) =>
      getHofbookNotesForDay(dayKey).map((note) => ({
        ...note,
        date: note.date || dayKey
      }))
    );
  ...
  const missingAttendance =
    getMissingAttendanceNames(todayKey);

  if (target) {
    target.innerHTML = `
      <button
        class="status-card clickable ${postIts.length ? "warn" : "ok"}"
        type="button"
        data-hofbook-cockpit-action="postits">

        <span class="status-card-icon">📌</span>

        <span class="status-card-copy">
          <span>Offene Post-its</span>
          <strong>${postIts.length}</strong>
          <small>
            ${
              postIts.length
                ? "Bleiben sichtbar, bis sie verteilt sind."
                : "Alles verteilt."
            }
          </small>
        </span>
      ...
      <button
        class="status-card clickable ${openEntries.length ? "warn" : "ok"}"
        type="button"
        data-hofbook-cockpit-action="open">
        ...
        <span class="status-card-copy">
          ...
          <small>Auf der sichtbaren Doppelseite</small>
        </span>
      </button>

      <button
        class="status-card clickable ${notes.length ? "warn" : "ok"}"
        type="button"
        data-hofbook-cockpit-action="notes">
      ...
      <button
        class="status-card clickable ${missingAttendance.length ? "danger" : "ok"}"
        type="button"
        data-hofbook-cockpit-action="attendance">
        ...
        <span class="status-card-copy">
          <span>Tageserfassung heute</span>

          <strong>
            ${
              missingAttendance.length
                ? `${missingAttendance.length} offen`
                : "Erledigt"
            }
          </strong>
  ...
  if (shelf) {
    shelf.innerHTML = postIts.length
      ? `
        <div class="hofbook-postit-shelf-head">
          <strong>Offene Post-its</strong>
          <small>
            Horizontal scrollen · Terminieren entfernt das Post-it aus der Ablage.
          </small>
        </div>

        <div class="hofbook-postit-shelf-track">
          ${postIts
            .map(buildHofbookPostItShelfCard)
            .join("")}
        </div>
      `
      : "";
...
function buildHofbookQuickNotesPreview(
  spreadNoteDays
) {
  if (!hofbookNotesExpanded) {
    return "";
  }

  const previewNotes =
    spreadNoteDays.flatMap((item) =>
      item.notes.map((note) => ({
        ...note,
        hofbookDayKey: item.dayKey
      }))
    );
  ...
  return `
    <div
      class="hofbook-quick-notes-preview"
      role="dialog"
      aria-label="Notizvorschau">

      <div class="hofbook-quick-notes-preview-head">
      ...
      <div class="hofbook-quick-notes-preview-list">
        ${previewNotes.map((note) => `
          <article
            class="hofbook-quick-note-preview-item"
            data-hofbook-open-notes-tab
            title="Notizen & Termine öffnen">
...
function renderHofbookQuickDashboard() {
  const target = $("#hofbookQuickDashboard");
  if (!target) return;

  const currentPayroll = getCurrentPayrollPeriod();

  const actualCounters =
    calculateOfficeCounters(currentPayroll);

  const plannedCounters =
    calculatePlannedCounters(currentPayroll);
  ...
  const rawPriceDate =
    state.settings.newPricesDate || "";

  const parsedPriceDate = rawPriceDate
    ? rawPriceDate.includes("T")
      ? new Date(rawPriceDate)
      : parseDateKey(rawPriceDate)
    : null;

  const priceDate =
    parsedPriceDate &&
    !Number.isNaN(parsedPriceDate.getTime())
      ? formatDate(parsedPriceDate)
      : "";

  const changedCount =
    priceStatus.changedMaterials.length;

  const priceMain = priceDate
    ? `${priceStatus.needsAttention ? "Neu seit" : "Stand"} ${priceDate}`
    : "Keine Liste markiert";

  const priceSmall = priceStatus.needsAttention
    ? changedCount
      ? `${changedCount} Material${changedCount === 1 ? "" : "ien"} geändert`
      : "Noch nicht bestätigt"
    : priceDate
      ? "Bereits bestätigt"
      : "Zum Eintragen anklicken";

  const nextEvent =
    getUpcomingEvents(1)[0] || null;

  const nextBirthday =
    getAllBirthdayDrawerItems()[0] || null;

  const eventText = nextEvent
    ? `${formatDate(parseDateKey(nextEvent.date))} · ${nextEvent.title || "Termin"}`
    : "Kein Termin";

  const birthdayText = nextBirthday
    ? `${formatDate(nextBirthday.next)} · ${nextBirthday.title}`
    : "Kein Geburtstag";

  const { leftKey, rightKey } =
  getHofbookBookSpreadKeys(getHofbookDate());

const spreadNoteDays = [leftKey, rightKey]
  .map((dayKey) => ({
    dayKey,
    notes: getHofbookNotesForDay(dayKey)
  }))
  .filter((item) => item.notes.length);

const noteCount = spreadNoteDays.reduce(
  (sum, item) => sum + item.notes.length,
  0
);

const noteMain = noteCount
  ? `${noteCount} Notiz${noteCount === 1 ? "" : "en"}`
  : "Keine Notiz";

const noteSmall = spreadNoteDays.length
  ? spreadNoteDays
      .map((item) =>
        formatDate(parseDateKey(item.dayKey))
      )
      .join(" + ")
  ...
  target.innerHTML = `
  <button
    class="hofbook-quick-card ${
      priceStatus.needsAttention
        ? "alert"
        : ""
    }"
    type="button"
    data-hofbook-quick-action="new-prices"
    title="${escapeHtmlAttr(
      priceStatus.title || priceSmall
    )}">

    <span class="hofbook-quick-copy">
      <span class="hofbook-quick-label">
        Preise
      </span>

      <span
        class="hofbook-quick-separator"
        aria-hidden="true">
        ·
      </span>

      <strong>
        ${escapeHtml(priceDate || "–")}
      </strong>
    </span>
  </button>
  ...
  <button
    class="hofbook-quick-card"
    type="button"
    data-hofbook-quick-action="yesim-hours"
    title="Yesims Stunden öffnen">
    ...
    <span class="hofbook-quick-copy">
      ...
      <strong>
        ${escapeHtml(
          `${yesimActual.hours}/${yesimPlanned.hours} Std.`
        )}
      </strong>
    </span>
  </button>
  ...
  <button
    class="hofbook-quick-card"
    type="button"
    data-hofbook-quick-action="upcoming"
    title="${escapeHtmlAttr(eventText)}">
    ...
    <span class="hofbook-quick-copy">
      ...
      <strong class="hofbook-quick-event-value">
  ${escapeHtml(eventText)}
</strong>
    </span>
  </button>

  <div class="hofbook-quick-notes-wrap">
    <button
      class="hofbook-quick-card hofbook-quick-notes"
      type="button"
      data-hofbook-quick-action="notes"
      title="Notizvorschau öffnen">
      ...
      <span class="hofbook-quick-copy">
        ...
        <strong>
          ${escapeHtml(String(noteCount))}
        </strong>
      </span>
    </button>

    ${buildHofbookQuickNotesPreview(
      spreadNoteDays
    )}
  </div>
`;
...
function buildHofbookQuickUpcomingHtml() {
  const events = getUpcomingEvents(5);
  const birthdays =
    getAllBirthdayDrawerItems().slice(0, 5);

  const eventRows = events.length
    ? events.map((event) => `
        <div class="drawer-list-item">
          <strong>${escapeHtml(event.title || "Termin")}</strong>
          <span>${escapeHtml(formatDate(parseDateKey(event.date)))}</span>
        </div>
      `).join("")
    : `
        <div class="drawer-list-item">
          <strong>Keine kommenden Termine</strong>
          <span>Aktuell ist kein Termin eingetragen.</span>
        </div>
      `;
  ...
  const birthdayRows = birthdays.length
    ...
    : `
        <div class="drawer-list-item">
          <strong>Keine Geburtstage eingetragen</strong>
          <span>Es wurden noch keine Geburtstage hinterlegt.</span>
        </div>
      `;

  return `
    <div class="drawer-section">
      <div class="drawer-eyebrow">Als Nächstes</div>
      <h3>Termine & Geburtstage</h3>
      ...
      <div class="hofbook-quick-drawer-actions">
        <button
          class="ghost"
          type="button"
          data-hofbook-quick-tab="notizen">
          Zu den Terminen
        </button>
        ...
        <button
          class="ghost"
          type="button"
          data-hofbook-quick-tab="mitarbeiterdaten">
          Zu den Geburtstagen
        </button>
      </div>
    </div>
...
function bindHofbookQuickDrawerLinks() {
  $$("#detailDrawer [data-hofbook-quick-tab]").forEach((button) => {
    button?.addEventListener("click", () => {
      activateTab(
        button.dataset.hofbookQuickTab || "hofbuch"
      );
...
const sickItems = getHofbookSickItems(currentKey);
...
${
 ...
 ${hofbookNotesExpanded ? buildHofbookNotesPopup(notes) : ""}
    ...
    title.classList.remove("hofbook-title-sunday", "hofbook-title-holiday", "hofbook-title-closed");
    title.innerHTML = `
      <span>${currentDate.toLocaleDateString("de-DE", {
        weekday: "long",
        day: "2-digit",
...
function getBossAbsenceCodeFromText(text) {
  const normalized = normalizeHofbookSearchText(text || "");
  const codes = new Set();

  if (
    normalized.includes("rg nicht da") ||
    normalized.includes("re nicht da") ||
    normalized.includes("renaldo nicht da")
  ) {
    codes.add("RG");
  }

  if (
    normalized.includes("tg nicht da") ||
    normalized.includes("tobias nicht da") ||
    normalized.includes("toni nicht da") ||
    normalized.includes("timon nicht da")
  ) {
    codes.add("TG");
  }
...
function getBossAbsenceCodeFromName(name) {
  const normalized = normalizeHofbookSearchText(name || "");

  if (normalized.includes("renaldo")) return "RG";

  if (
    normalized.includes("tobias") ||
    normalized.includes("toni") ||
...
function getBossAbsenceSourceEntries(dayKey, code) {
  return (state.hofbookEntries || []).filter((entry) => {
  if (entry.type === "postit") return false;
  if (entry.date !== dayKey) return false;
    const codes = getBossAbsenceCodeFromText(entry.text || "");
    return codes.includes(code);
  });
}

function isHofbookBossAbsenceEntry(entry) {
  return getBossAbsenceCodeFromText(entry.text || "").length > 0;
}

function getHofbookBossAbsenceCodes(dayKey) {
  const codes = new Set();

  (state.externalBirthdays || []).forEach((boss) => {
    const entry = getManagementAttendanceEntry(boss.id, dayKey);
    if (entry.status === "U" || entry.status === "K") {
      const code = getBossAbsenceCodeFromName(boss.name || "");
      if (code) codes.add(code);
    }
  });

  (state.hofbookEntries || []).forEach((entry) => {
  if (entry.type === "postit") return;
  if (entry.date !== dayKey) return;

  getBossAbsenceCodeFromText(
    entry.text || ""
  ).forEach(
    (code) => codes.add(code)
  );
  ...
  return ["RG", "TG"].filter((code) => codes.has(code));
}
...
function getHofbookEventsForDay(dayKey) {
  return (state.events || [])
    .flatMap((event) => getEventOccurrencesInRange(event, parseDateKey(dayKey), parseDateKey(dayKey)))
...
function getHofbookBirthdayItems(dayKey) {
  ...
  (state.employees || [])
    .filter((employee) =>
      ...
      employee.birthday &&
      isBirthdayOnDay(employee.birthday, day)
    )
    .forEach((employee) => {
      birthdays.push({
  ...
  (state.externalBirthdays || [])
    .filter((item) =>
      item.birthday &&
      isBirthdayOnDay(item.birthday, day)
    )
    .forEach((item) => {
      birthdays.push({
        id: `external-${item.id}`,
...
function getHofbookPostItsForSpread(spreadKey) {
  return (state.hofbookEntries || [])
    .filter((entry) => entry.type === "postit")
    .filter((entry) => {
      const entrySpreadKey =
        entry.spreadKey ||
        getHofbookBookSpreadStartKey(
          entry.date || spreadKey
        );

      return entrySpreadKey === spreadKey;
    })
    .sort((a, b) =>
...
function carryOpenHofbookPostItsToToday() {
  const todayKey = dateKey(new Date());

  const todaySpreadKey =
    getHofbookBookSpreadStartKey(todayKey);
  ...
  (state.hofbookEntries || []).forEach((entry) => {
    ...
    // Nur echte offene Post-its übernehmen
    ...
    // Nur ältere Post-its übernehmen.
    // Beim normalen Blättern passiert gar nichts.
    if (oldSpreadKey >= todaySpreadKey) return;

    entry.spreadKey = todaySpreadKey;
    entry.date = todaySpreadKey;
    entry.updatedAt = new Date().toISOString();

    // x und y bleiben absichtlich unverändert.
    // Das Post-it behält seine Position.
    movedCount += 1;
  });
...
function buildHofbookPostIt(entry) {
  const rawX = Number(entry.x);
  const rawY = Number(entry.y);
  ...
  return `
    <article
      class="hofbook-postit${isDraft ? " editing" : ""}"
      data-hofbook-postit-id="${escapeHtmlAttr(entry.id)}"
      style="left:${x}%; top:${y}%;">
      
      <div
        class="hofbook-postit-grip"
      ...
      ${
        isDraft
          ? `
            <textarea
              class="hofbook-postit-draft-input"
              data-hofbook-postit-draft-input="${escapeHtmlAttr(entry.id)}"
              placeholder="Notiz eingeben..."
              rows="5">${escapeHtml(entry.text || "")}</textarea>
          `
          : `
            <div class="hofbook-postit-text">
              ${escapeHtml(entry.text || "").replace(/\n/g, "<br>")}
            </div>
          `
      }

      <div class="hofbook-postit-actions">
        ${
          isDraft
            ? `
              <button
                type="button"
                title="Post-it speichern"
                data-hofbook-postit-save="${escapeHtmlAttr(entry.id)}">
                &#10003;
              </button>
              ...
              <button
                type="button"
                title="Abbrechen"
                data-hofbook-postit-cancel="${escapeHtmlAttr(entry.id)}">
                &times;
              </button>
            `
            : `
...
function saveHofbookPostItDraft(id) {
  const entry = (state.hofbookEntries || []).find(
    (item) =>
      item.id === id &&
      item.type === "postit"
  );
  ...
  const input = document.querySelector(
    `[data-hofbook-postit-draft-input="${id}"]`
  );
...
function cancelHofbookPostItDraft(id) {
  state.hofbookEntries =
    (state.hofbookEntries || []).filter(
      (item) => item.id !== id
    );
...
function buildHofbookPostIts(spreadKey) {
  return getHofbookPostItsForSpread(spreadKey)
    .map(buildHofbookPostIt)
    .join("");
}

function addHofbookPostIt() {
  if (!Array.isArray(state.hofbookEntries)) {
    state.hofbookEntries = [];
  }
  ...
  setTimeout(() => {
    const input = document.querySelector(
      `[data-hofbook-postit-draft-input="${draftId}"]`
    );
...
function editHofbookPostIt(id) {
  ...
  if (!text) {
    showToast(
      "Das Post-it darf nicht leer sein.",
      "error"
    );
    return;
  }
...
function deleteHofbookPostIt(id) {
  const entry = (state.hofbookEntries || []).find(
    (item) =>
      item.id === id &&
      item.type === "postit"
...
function startHofbookPostItDrag(event) {
  const postIt = event.target.closest(
    ".hofbook-postit[data-hofbook-postit-id]"
  );
  ...
  const entry = (state.hofbookEntries || []).find(
    (item) =>
      item.id ===
        postIt.dataset.hofbookPostitId &&
      item.type === "postit"
  );
  ...
  hofbookPostItDrag = {
    ...
    element: postIt,
    offsetX:
      event.clientX - postItRect.left,
    offsetY:
      event.clientY - postItRect.top,
    moved: false
  };

  postIt.classList.add("dragging");

  postIt.setPointerCapture?.(
    event.pointerId
  );
...
function moveHofbookPostItDrag(event) {
  const drag = hofbookPostItDrag;
  ...
  const spreadRect =
    drag.spread.getBoundingClientRect();

  const postItWidth =
    drag.element.offsetWidth;

  const postItHeight =
    drag.element.offsetHeight;

  const maxLeft = Math.max(
    0,
    spreadRect.width - postItWidth
  );

  const maxTop = Math.max(
    0,
    spreadRect.height - postItHeight
  );
  ...
  const x = spreadRect.width
    ? (left / spreadRect.width) * 100
    : 0;

  const y = spreadRect.height
    ? (top / spreadRect.height) * 100
    : 0;
...
function endHofbookPostItDrag(event) {
  const drag = hofbookPostItDrag;
  ...
  try {
    ...
    // Pointer war bereits freigegeben.
  }

  hofbookPostItDrag = null;
...
function getHofbookPageStatus(dayKey) {
  const currentDate = parseDateKey(dayKey);
  const holidayMap = buildHolidayMapForRange(currentDate, currentDate);
  const holidayName = holidayMap[dayKey] || "";
  const specialDay = getSpecialOfficeDay(dayKey);
  const isSunday = currentDate.getDay() === 0;
  const isClosed = specialDay.mode === "closed";
...
function buildHofbookBookDateBadges(dayKey) {
  const trashItems = getWasteEntries().filter((item) => getWasteMarkerDate(item.date) === dayKey);
  const bossAbsenceCodes = getHofbookBossAbsenceCodes(dayKey);
  const todaysEvents = getHofbookEventsForDay(dayKey);
  const officeDutyNames = getHofbookOfficeDutyNames(dayKey);
  const manualBadges = getHofbookManualBadges(dayKey);
  const vacationItems = getHofbookVacationItems(dayKey);
  const sickItems = getHofbookSickItems(dayKey);
  const birthdayItems = getHofbookBirthdayItems(dayKey);

  return `
    ${manualBadges.map(buildHofbookBookManualBadge).join("")}

    ${
      trashItems.length ?
         `<button class="hofbook-book-badge waste" type="button" data-hofbook-open-waste>🗑 ${escapeHtml(trashItems.map((item) => item.type).join(" + "))}</button>`
        : ""
    }
  ...
  birthdayItems.length ?
     `<span
        class="hofbook-book-badge birthday"
        title="${escapeHtmlAttr(
          birthdayItems
            .map((item) => getBirthdayTooltip(item, parseDateKey(dayKey)))
            .join(" · ")
        )}">
        🎂 ${escapeHtml(
          birthdayItems
            .map((item) => getFirstName(item.name || "Ohne Namen"))
            .join(", ")
        )}
      </span>`
    : ""
    ...
    ${bossAbsenceCodes.map((code) => {
      const sourceEntries = getBossAbsenceSourceEntries(dayKey, code);
      const canDelete = sourceEntries.length > 0;

      return `
        <span class="hofbook-book-badge absence" title="${escapeHtmlAttr(`${code} nicht da!`)}">
          ${escapeHtml(`${code} nicht da!`)}
          ${
            canDelete ?
              `<button
                  class="hofbook-book-badge-delete"
                  type="button"
                  title="${escapeHtmlAttr(`${code} nicht da! l\u00f6schen`)}"
                  data-hofbook-delete-boss-absence="${escapeHtmlAttr(code)}"
                  data-hofbook-delete-boss-absence-date="${escapeHtmlAttr(dayKey)}">&times;</button>`
              : ""
          }
        </span>
      `;
    ...
    ${
      officeDutyNames.length ?
         `<button class="hofbook-book-badge office" type="button" data-hofbook-open-office title="${escapeHtmlAttr(officeDutyNames.join(" + "))}">🏢 ${escapeHtml(officeDutyNames.join(" + "))}</button>`
        : ""
    }

    ${
      vacationItems.length ?
         `<span class="hofbook-book-badge vacation" title="${escapeHtmlAttr(vacationItems.map((item) => item.name).join(", "))}">🌴 Urlaub: ${escapeHtml(vacationItems.map((item) => item.shortName).join(", "))}</span>`
        : ""
    }

    ${
      sickItems.length ?
         `<span class="hofbook-book-badge sick" title="${escapeHtmlAttr(sickItems.map((item) => item.name).join(", "))}">🤒 Krank: ${escapeHtml(sickItems.map((item) => item.shortName).join(", "))}</span>`
        : ""
    }

    ${
      todaysEvents.length ?
 `<button class="hofbook-book-badge event" type="button" data-open-events title="${escapeHtmlAttr(todaysEvents.map((event) => event.title || "Termin").join(", "))}">📌 ${escapeHtml(todaysEvents.length === 1 ? (todaysEvents[0].title || "Termin") : `${todaysEvents.length} Termine`)}</button>`
        : ""
    }
  `;
}
...
function buildHofbookBookEntryLine(entry) {
 ...
 ${entry.done ? "✓" : ""}
  ...
  class="ghost"
  type="button"
  title="Wieder als Post-it ablegen"
  data-hofbook-to-postit="${escapeHtmlAttr(entry.id)}">
  📌
</button>
...
function buildHofbookBookPage(
  dayKey,
  pageSide = "left"
) {
  const page =
    getHofbookBookPageTitle(dayKey);
  ...
  const pageStatus =
    getHofbookPageStatus(dayKey);

  const dateBadges =
    buildHofbookBookDateBadges(dayKey);

  const emptyLineCount =
    Math.max(8, 16 - entries.length);

  return `
    <section
      class="hofbook-book-page ${pageSide} ${isToday ? "today" : ""} ${
        pageStatus.type
          ? `hofbook-page-${pageStatus.type}`
          : ""
      }"
      data-hofbook-page-date="${escapeHtmlAttr(dayKey)}">

      <header class="hofbook-book-page-head">
        <div class="hofbook-book-dateblock">
          <span class="hofbook-book-weekday">
            ${escapeHtml(page.weekday)}
          </span>

          <strong class="hofbook-book-daynumber">
            ${escapeHtml(page.dayNumber)}
          </strong>

          <div class="hofbook-book-meta">
            <span>
              ${escapeHtml(page.month)}
              ${escapeHtml(page.year)}
              · KW ${escapeHtml(page.kw)}
            </span>
            ...
            ${
              pageStatus.label
                ? `
                  <span
                    class="hofbook-page-status-badge ${escapeHtmlAttr(
                      pageStatus.type
                    )}">
                    ${escapeHtml(pageStatus.label)}
                  </span>
                `
                : ""
            }
      ...
      <div class="hofbook-book-lines">
        ${
          entries.length
            ? entries
                .map(buildHofbookBookEntryLine)
                .join("")
            : `
              <div class="hofbook-book-empty-text">
                Hier klicken und den ersten Eintrag schreiben …
              </div>
            `
        }

        ${buildHofbookEmptyBookLines(
          emptyLineCount,
          dayKey
        )}
      </div>
...
function renderHofbookPaper() {
  ...
  paper.innerHTML = `
    <div class="hofbook-book-stage">
      ...
      <div
  class="hofbook-book-spread"
  data-hofbook-spread-key="${escapeHtmlAttr(leftKey)}">

  ${buildHofbookBookPage(leftKey, "left")}
  ${buildHofbookBookPage(rightKey, "right")}

  ${buildHofbookPostIts(leftKey)}
</div>
...
function moveHofbookEntryToPostIt(id) {
  const entry = (state.hofbookEntries || []).find(
    (item) =>
      item.id === id &&
      item.type !== "postit"
  ...
  showConfirm(
    "Eintrag wieder als Post-it ablegen",
    () => {
      const spreadKey =
        getHofbookBookSpreadStartKey(
          getHofbookDate()
      ...
      showToast(
        "Eintrag wurde wieder als Post-it abgelegt.",
        "success"
      );
    }
  );
...
function openHofbookMoveModal(id) {
  ...
  if (!modal || !input) {
    console.error(
      "Hofbuch-Verschieben nicht möglich: Modal oder Datumsfeld fehlt.",
      {
        modal,
        input,
        entryId: id
  ...
  if (title) {
    title.textContent = isPostIt
      ? "Post-it terminieren"
      : "Eintrag verschieben";
  }

  if (saveButton) {
    saveButton.textContent = isPostIt
      ? "Ins Hofbuch übernehmen"
      : "Verschieben";
  }
...
function saveHofbookMoveDate() {
  ...
  showToast(
    wasPostIt
      ? "Post-it ins Hofbuch übernommen."
...
function getFeedbackTypeMeta(type) {
  const map = {
    bug: {
      icon: "🐞",
      label: "Fehler"
...
function getFeedbackAreaFromTab(tabId) {
  const map = {
    hofbuch: "Hofbuch",
    anwesenheit: "Tageserfassung",
    preislisten: "Preislisten",
...
function openFeedbackModal(editId = "") {
  ...
  const typeInput = $("#feedbackTypeInput");
  const areaInput = $("#feedbackAreaInput");
  const messageInput = $("#feedbackMessageInput");
  const urgentInput = $("#feedbackUrgentInput");
  const submitButton = $("#feedbackModalSubmit");
  const title = modal?.querySelector("h3");
  ...
  areaInput.value =
    item?.area ||
    getFeedbackAreaFromTab(
      lastNonFeedbackTabId
    );
...
function canManageAllFeedback() {
  const role = String(
    currentUserProfile?.role || ""
  ).toLowerCase();

  return FEEDBACK_MANAGER_ROLES.has(role);
}
...
async function loadFeedbackItems() {
  ...
  try {
    ...
    let query = supabaseClient
      .from("app_feedback")
      .select(
        "id, created_at, updated_at, type, area, message, urgent, status, app_version, user_id, user_email, active_tab, context_data"
      )
      .order("created_at", {
        ascending: false
      });
    ...
    // Admin sieht alles.
    // Alle anderen nur ihr eigenes Feedback.
    ...
    feedbackItems = Array.isArray(data)
      ? data
      : [];
...
function renderFeedbackPage() {
  ...
  const visibleItems = feedbackItems.filter(
    (item) => {
      if (
        feedbackActiveFilter !== "all" &&
        item.status !== feedbackActiveFilter
      ) {
        return false;
      }
  ...
  const countAll = feedbackItems.length;

  const countNew = feedbackItems.filter(
    (item) => item.status === "new"
  ).length;

  const countProgress = feedbackItems.filter(
    (item) => item.status === "in_progress"
  ).length;

  const countDone = feedbackItems.filter(
    (item) => item.status === "done"
  ).length;

  if ($("#feedbackCountAll")) {
    $("#feedbackCountAll").textContent =
      String(countAll);
  }

  if ($("#feedbackCountNew")) {
    $("#feedbackCountNew").textContent =
      String(countNew);
  }

  if ($("#feedbackCountProgress")) {
    $("#feedbackCountProgress").textContent =
      String(countProgress);
  }

  if ($("#feedbackCountDone")) {
    $("#feedbackCountDone").textContent =
      String(countDone);
  }

  $$(".feedback-filter-btn").forEach(
    (button) => {
      button.classList.toggle(
        "active",
        button.dataset.feedbackFilter ===
          feedbackActiveFilter
      );
    }
  );
  ...
  list.innerHTML = visibleItems
    .map((item) => {
      ...
      return `
        <article
          class="feedback-card
            status-${escapeHtmlAttr(item.status || "new")}
            ${item.urgent ? "urgent" : ""}">
          ...
          <div class="feedback-card-head">
            <div class="feedback-card-title">
              ...
              <div>
                ...
                <span class="feedback-area">
                  ${escapeHtml(item.area || "Allgemein")}
                </span>
              </div>
            ...
            <div class="feedback-card-badges">
              ${
                item.urgent
                  ? `
                    <span class="feedback-urgent-badge">
                      🚨 Dringend
                    </span>
                  `
                  : ""
          ...
          <div class="feedback-card-footer">
            ...
            <div class="feedback-card-controls">
  <button
    class="ghost"
    type="button"
    data-feedback-edit-id="${escapeHtmlAttr(item.id)}">
    Bearbeiten
  </button>
  ...
  canManageAllFeedback()
    ? `
      <label class="feedback-status-control">
        <span>Status</span>

        <select
          data-feedback-status-id="${escapeHtmlAttr(item.id)}">
    ...
    `
    : `
      <div class="feedback-status-control">
        <span>Status</span>
        <strong>
          ${escapeHtml(statusLabel)}
        </strong>
...
async function submitFeedback() {
  ...
  if (!message) {
    showToast(
      "Bitte beschreibe dein Feedback.",
      "error"
    );
...
async function updateFeedbackStatus(
  ...
  try {
    ...
    console.error(
      "Feedback-Status konnte nicht geändert werden:",
      error
    );
...
function bindFeedback() {
  $("#feedbackNewBtn")?.addEventListener(
    "click",
    openFeedbackModal
  );

  $("#feedbackReloadBtn")?.addEventListener(
    "click",
    loadFeedbackItems
  );

  $("#feedbackSearchInput")?.addEventListener(
    "input",
    (event) => {
      feedbackSearchTerm =
        event.target.value || "";
  ...
  $("#feedbackModalCancel")?.addEventListener(
    "click",
    closeFeedbackModal
  );

  $("#feedbackModalCloseX")?.addEventListener(
    "click",
    closeFeedbackModal
  );

  $("#feedbackModalSubmit")?.addEventListener(
    "click",
    submitFeedback
  );
  ...
  (event) => {
    const editButton = event.target.closest(
      "[data-feedback-edit-id]"
    );

    if (!editButton) return;

    openFeedbackModal(
      editButton.dataset.feedbackEditId
    );
  }
);
  ...
  $("#feedbackList")?.addEventListener(
    ...
    (event) => {
      const select = event.target.closest(
        "[data-feedback-status-id]"
      );

      if (!select) return;

      updateFeedbackStatus(
        select.dataset.feedbackStatusId,
        select.value
      );
    }
  );
...
function renderOwnPurchaseHistoryChart() {
  ...
  const chartPoints = monthly.map((bucket, index) => ({
    x: xForIndex(index),
    y: yForPrice(bucket.avg),
    bucket
  }));
  ...
  function drawSmoothPath(pointsToDraw) {
    ...
    for (let i = 0; i < pointsToDraw.length - 1; i += 1) {
  ...
  function drawSmoothArea(pointsToDraw) {
    if (!pointsToDraw.length) return;

    ctx.beginPath();
    ctx.moveTo(pointsToDraw[0].x, baseY);
    ctx.lineTo(pointsToDraw[0].x, pointsToDraw[0].y);
    ...
    if (pointsToDraw.length > 1) {
      ...
      ctx.lineTo(pointsToDraw[0].x, baseY);
    }
  ...
  // horizontale Gridlinien
  ctx.save();
  ctx.strokeStyle = "#e8edf3";
  ctx.lineWidth = 1;
  ctx.fillStyle = "#7d8a98";
  ...
  for (let i = 0; i <= 4; i += 1) {
    ...
    ctx.fillText(formatEuroPerKg(value), padding.left - 12, y + 4);
  }
  ctx.restore();

  // Monats-Spannen (dezent statt dick)
  ctx.save();
  ctx.strokeStyle = "rgba(227, 16, 16, 0.16)";
  ctx.lineWidth = 5;
  ctx.lineCap = "round";
  ...
  monthly.forEach((bucket, index) => {
    ...
    if (Math.abs(yMaxLine - yMinLine) < 8) return;
  ...
  areaGradient.addColorStop(0, "rgba(227, 16, 16, 0.18)");
  areaGradient.addColorStop(1, "rgba(227, 16, 16, 0.02)");
  ctx.fillStyle = areaGradient;
  drawSmoothArea(chartPoints);
  ctx.fill();
  ctx.restore();

  // Durchschnittslinie smooth
  ctx.save();
  ctx.strokeStyle = "#e31010";
  ctx.lineWidth = 4;
  ctx.lineCap = "round";
  ctx.lineJoin = "round";
  ctx.shadowColor = "rgba(227, 16, 16, 0.12)";
  ctx.shadowBlur = 10;
  drawSmoothPath(chartPoints);
  ctx.stroke();
  ctx.restore();

  // Punkte + Monatslabels
  const hitPoints = [];
  const labelStep = monthly.length > 18 ? 2 : 1;
  ...
  chartPoints.forEach((point, index) => {
    ...
    // Monatslabel etwas ruhiger
    if (index % labelStep === 0 || index === monthly.length - 1) {
      ctx.save();
      ctx.fillStyle = "#7d8a98";
      ctx.font = "11px Segoe UI, Arial";
      ctx.textAlign = "center";
      ctx.fillText(formatMonthLabel(point.bucket.monthKey), point.x, height - 28);
      ctx.restore();
    }
  ...
  // Nur erster und letzter Wert
  const first = chartPoints[0];
  const last = chartPoints[chartPoints.length - 1];

  [
    { point: first, text: formatEuroPerKg(first.bucket.avg), align: "left", offsetX: 10 },
    { point: last, text: formatEuroPerKg(last.bucket.avg), align: "left", offsetX: 10 }
  ].forEach((item) => {
    ctx.save();
    ctx.fillStyle = "#1b2a38";
    ctx.font = "700 12px Segoe UI, Arial";
    ctx.textAlign = item.align;
    ctx.fillText(item.text, item.point.x + item.offsetX, item.point.y - 12);
    ctx.restore();
  });
  ...
  ctx.textAlign = "left";
  ctx.fillText(`Preisentwicklung: ${monthly[0].label || "Material"}`, padding.left, 28);
  ctx.restore();

  // Info rechts oben
  const lastDiff = monthly.length >= 2 ? last.bucket.avg - monthly[monthly.length - 2].avg : 0;

  ctx.save();
  ctx.fillStyle = lastDiff >= 0 ? "#2f7553" : "#d64545";
  ctx.font = "700 13px Segoe UI, Arial";
  ctx.textAlign = "right";
  ctx.fillText(
    `${monthly.length} Monate · letzter Ø ${formatEuroPerKg(last.bucket.avg)} · ${lastDiff >= 0 ? "+" : ""}${formatEuroPerKg(lastDiff)}`,
    width - padding.right,
    28
  );
  ctx.restore();
...
function renderContainers() {
  ...
  if (containerListMode === "recent") {
    containerVisibleRows =
      getRecentContainerRows();
  }
...
function renderContainerTable(rows) {
 ...
 const list = Array.isArray(rows) ? rows : [];
  ...
  if (!state.containers.length) {
    info.textContent =
      "Noch keine Container geladen.";
  } else if (!list.length) {
    info.textContent =
      `${state.containers.length} Container gespeichert. Suche nach Nummer oder Gewicht.`;
  } else if (
    containerListMode === "recent"
  ) {
    info.textContent =
      `Zuletzt gesucht: ${list.length} Container · insgesamt ${state.containers.length} gespeichert.`;
  } else {
    info.textContent =
      `${list.length} Treffer angezeigt. Insgesamt ${state.containers.length} Container gespeichert.`;
  }
}
...
function getRecentContainerNumbers() {
  if (
    !Array.isArray(
      state.settings.containerRecentNumbers
    )
  ) {
    state.settings.containerRecentNumbers =
      [];
  ...
  return state.settings.containerRecentNumbers;
}

function getRecentContainerRows() {
  return getRecentContainerNumbers()
    .map((number) =>
      state.containers.find(
        (container) =>
...
function rememberRecentContainers(
  containers
) {
  const newNumbers =
    containers
      .map((container) =>
        String(container.number || "").trim()
      )
      .filter(Boolean);

  const oldNumbers =
    getRecentContainerNumbers();
...
function searchContainers() {
  const number =
    (
      $("#containerSearchNumberInput")
        ?.value || ""
    ).trim();

  const weight =
    (
      $("#containerSearchWeightInput")
        ?.value || ""
    ).trim();
  ...
  const sortedMatches =
    matches.sort(
      sortContainersByNumber
    );

  rememberRecentContainers(
    sortedMatches
  );
...
function renderOfficeCounterMiniStats(counter) {
 ...
 state.officePlan[date][role === "primary" ? "primaryEmployeeId" : "secondaryEmployeeId"] = value;
  ...
  function getAttendanceEmployeeInitials(name) {
  return String(name || "")
    .trim()
    .split(/\s+/)
    .filter(Boolean)
...
function getAttendanceTrackSymbol(status) {
  return {
    A: "✓",
    U: "U",
    K: "K",
...
function renderAttendanceMonthHeader(
  days,
  holidays
) {
  const selectedKey =
...
function renderEmployeeMonthTrack(
  employeeId,
  days,
  holidays
) {
...
const entryDate =
  getEmployeeEntryDate(employee);

const exitDate =
  getEmployeeExitDate(employee);
...
const isEmploymentBlocked =
  isBeforeEmployment ||
  isAfterEmployment;

const employmentHint =
  isBeforeEmployment
    ...
    : isAfterEmployment
      ? exitDate
        ? `Inaktiv ab ${formatDate(
            exitDate
          )}`
        : "Inaktiv"
      : "";

const isMixedVacation =
  isAttendanceVacationMixed(entry);

      const vacationBadge =
        getVacationValueBadge(entry);

      const isNonCountedVacationDay =
        status === "U" &&
        isPartTimeNonCountedVacationDay(
          employeeId,
          day
        );

      const label =
        isNonCountedVacationDay
          ? "Urlaub – zählt nicht als Urlaubstag"
          : isMixedVacation
            ? getMixedAttendanceTrackLabel(
                entry
              )
...
${
  isEmploymentBlocked
    ? "employment-blocked-track"
    : ""
}
${
  isBeforeEmployment
    ? "employment-not-started-track"
    : ""
}
${
  isAfterEmployment
    ? "employment-ended-track"
    : ""
}
${
  isSelected
          ...
          aria-label="${escapeHtmlAttr(
            tooltip
          )}"
          data-track-date="${key}"
          data-vacation-badge="${escapeHtmlAttr(
            vacationBadge
          )}">
...
function getMixedAttendanceTrackLabel(entry) {
 ...
 return `Anwesend + ${formatDayCount(value)} Urlaubstag${value === 1 ? "" : "e"}`;
  ...
  (state.employees || [])
    .filter((employee) =>
      isEmployeeVisibleInAttendanceMonth(
        employee,
        monthView
      )
    );
...
const isSaturdayRequirement =
  attendanceCompletion
    .isSaturdayRule;

  if (hints) {
    hints.innerHTML = `
      <span class="attendance-legend-item blank">
        <i></i>
        Leer
      </span>

      <span class="attendance-legend-item present">
        <i></i>
        Anwesend
      </span>

      <span class="attendance-legend-item vacation">
        <i></i>
        Urlaub
      </span>

      <span class="attendance-legend-item sick">
        <i></i>
        Krank
      </span>

      <span class="attendance-legend-item other">
        <i></i>
        Sonstiges
      </span>
      ...
      ${
        isBlockedDay
          ? `
            <span class="attendance-legend-warning">
              ${escapeHtml(
                isClosedDay
                  ? "Geschlossen im Büroplan"
                  : holidayName ||
                    "Sonntag"
              )}
      ...
      ${
  isSaturdayRequirement
    ? `
      <span class="attendance-legend-warning">
        Samstag · eine anwesende Person aus Büro
        und eine aus Lager reichen
      </span>
    `
    : ""
}
  ...
  board.innerHTML = `
    <div class="attendance-day-overview">
      ...
      <div
        ...
        <span class="attendance-progress-icon">
          ${
            isBlockedDay
              ? "–"
              : isComplete
        ...
        <span>
          <strong>
            ${
              isBlockedDay
                ? "Keine Erfassung möglich"
                : `${completedRequired} von ${requiredCount} erforderlich erfasst`
            }
          </strong>
          ...
          <small>
            ${
              isBlockedDay
                ...
                : isSaturdayRequirement
  ? isComplete
    ? "Samstagsbesetzung vollständig: Büro und Lager."
    : "Eine Büro-Person und eine Lager-Person auf Anwesend setzen."
  : isComplete
    ? "Die Tageserfassung ist vollständig."
    : "Fehlende Status direkt in der Mitarbeiterzeile ergänzen."
            }
          </small>
        </span>
      </div>
    ...
    <div class="attendance-matrix-shell">
      <div
        class="attendance-matrix"
        style="
          --attendance-days:${days.length};
          --attendance-matrix-width:${matrixWidth}px;
        ">

        <div class="attendance-matrix-head">
          <div class="attendance-employee-head">
            <span>
              Mitarbeiter und Status
            </span>
...
const employeeEntryDate =
  getEmployeeEntryDate(emp);

const employeeExitDate =
  getEmployeeExitDate(emp);

const isBeforeEmployment =
  !!employeeEntryDate &&
  dateOnly(selectedDateObj) <
    dateOnly(employeeEntryDate);
...
const isNonCountedVacationDay =
              ...
              const canShowAttendanceDetails =
  !isAttendanceDisabled &&
  (
    canHaveVacationValue(
      entry.status
    ) ||
    entry.status === "S"
  );
...
const hasAttendanceDetails =
  getVacationDayValue(entry) > 0 ||
  Boolean(
    String(entry.note || "").trim()
  );

const attendanceDetailsOpen =
  canShowAttendanceDetails &&
  attendanceDetailsOpenEmployeeId ===
    emp.id;
            ...
            return `
              <div
                class="
                  ...
                  ${
                    isNonCountedVacationDay
                      ? "non-counted-vacation-attendance"
                      : ""
                  }
                  ${
                    isSundayDay
...
${
  isEmploymentBlocked
    ? "employment-blocked-attendance"
    : ""
}
${
  isBeforeEmployment
    ? "employment-not-started-attendance"
    : ""
}
${
  isAfterEmployment
    ? "employment-ended-attendance"
    : ""
}
${
  attendanceDetailsOpen
    ...
    : ""
                ...
                "
                data-employee-id="${escapeHtmlAttr(
                  emp.id
                )}">

                <div class="attendance-employee-cell">
                  <div class="attendance-employee-identity">
                    <span class="attendance-avatar">
                      ${escapeHtml(
                        getAttendanceEmployeeInitials(
                          emp.name
                        )
                      )}
                    </span>

                    <span class="attendance-employee-copy">
                      <strong>
                        ${escapeHtml(emp.name)}
                      </strong>
...
${
  ...
  <select
    ...
    data-employee="${escapeHtmlAttr(
      emp.id
    )}"
    aria-label="Status für ${escapeHtmlAttr(
      emp.name
    )}"
    ${
  isAttendanceDisabled
    ...
    canShowAttendanceDetails
      ? `
        <button
          ...
          "
          type="button"
          data-attendance-details-toggle
          data-employee="${escapeHtmlAttr(
            emp.id
          )}"
          aria-expanded="${
    ...
    canShowAttendanceDetails
      ? `
        <div
          ...
          <div class="attendance-row-slideout-inner">
            ${
              canHaveVacationValue(
                entry.status
              )
                ? `
                  <label class="daily-vacation-value">
                    <span>
                      ${
                        entry.status === "A"
                          ? "Zusätzlicher Urlaub"
            ...
            ${
              entry.status === "S"
                ? `
                  <label class="attendance-note-field">
                    <span>Notiz</span>

                    <input
                      class="daily-note"
                      type="text"
                      placeholder="Grund oder Hinweis eintragen …"
            ...
            <button
              class="attendance-details-close"
              type="button"
              data-attendance-details-close
              data-employee="${escapeHtmlAttr(
                emp.id
              )}"
              title="Zusatzangaben schließen"
              aria-label="Zusatzangaben schließen">
              ×
            </button>
          </div>
        </div>
...
function getManagementPlannerEntry(person, key) {
  ...
  const hofbookEntry = getHofbookManagementAttendanceEntry(person, key);
...
function getVacationValueBadge(entry) {
 ...
 node.dataset.activeGroup = employee.active !== false ? "active" : "inactive";
          ...
          setField(node, "personnelNumber", employee.personnelNumber || "");
...
function saveEmployeeCardLegacy(node, year = new Date().getFullYear()) {
  ...
  emp.personnelNumber = getField(node, "personnelNumber").trim();
  ...
  const personnelNumberInput = $("#employeeEditPersonnelNumberInput");
  ...
  const exitDateInput = $("#employeeEditExitDateInput");
  const exitDateField = $("#employeeEditExitDateField");
  ...
  if (
    ...
    !birthdayInput ||
    !activeInput ||
    !vacationAllowanceInput ||
    !vacationCarryoverInput ||
    !saveBtn ||
    !deleteBtn
  ) {
    return;
  ...
  vacationCarryoverInput.value = String(
    getVacationCarryoverForYear(emp, year)
  );

  const syncExitDateVisibility = () => {
    const isInactive = activeInput.value === "false";

    exitDateField?.classList.toggle("hidden", !isInactive);
  ...
  syncExitDateVisibility();

  activeInput.onchange = syncExitDateVisibility;
  ...
  saveBtn.onclick = () => {
    const nextActive = activeInput.value === "true";

    if (!nextActive && !exitDateInput.value) {
      showToast(
        "Bitte bei inaktiven Mitarbeitern ein Austrittsdatum eintragen.",
        "error"
      );
      exitDateInput.focus();
      return;
  ...
  deleteBtn.onclick = () => {
    ...
    Object.keys(state.officePlan).forEach((date) => {
      if (state.officePlan[date].primaryEmployeeId === id) {
        state.officePlan[date].primaryEmployeeId = "";
      }

      if (state.officePlan[date].secondaryEmployeeId === id) {
        state.officePlan[date].secondaryEmployeeId = "";
      }
    });

    saveState({
      remote: ["employees", "attendance", "officePlan"]
    });
...
input.onkeydown = null;
 ...
 setField(node, "date", item.date ? formatDate(parseDateKey(item.date)) : "");
        ...
        setField(node, "recurrence", getEventRecurrenceLabel(item.recurrence));
...
deleteButton?.addEventListener("click", () => deleteEvent(item.id));
  ...
  item.recurrence = normalizeEventRecurrence(getField(node, "recurrence"));
  ...
  function openEventEditModal(id) {
    ...
    const recurrenceInput = $("#eventEditRecurrenceInput");
    const notesInput = $("#eventEditNotesInput");
    const deleteBtn = $("#eventEditDelete");
    const saveBtn = $("#eventEditSave");
    if (!item || !modal || !titleInput || !dateInput || !typeInput || !recurrenceInput || !notesInput || !deleteBtn || !saveBtn) return;
    ...
    saveBtn.onclick = () => {
      ...
      item.recurrence = recurrenceInput.value || "";
...
const nextDeadline =
  getVehicleNextDeadline(vehicle);
...
if (vehicle.active === false) {
  ...
  const deadlineDate =
    parseMonthKey(nextDeadline.date);
...
return vacationValue > 0 && !isSunday && !isHoliday ?
 ...
 days: quota ? Object.values(usedByWeek).reduce((sum, value) => sum + value, 0) : daysCount,
  ...
  const parts = [
    {
      label: "A",
      name: "Anwesend",
      value: Number(stats.A || 0),
      color: "#2f7553"
    },
    {
      label: "U",
      name: "Urlaub",
      value: Number(stats.U || 0),
      color: "#4778ae"
    },
    {
      label: "K",
    ...
    .map((part) => {
      ...
      return `
        <circle
          ...
          stroke-dasharray="
            ${visibleLength.toFixed(3)}
            ${hiddenLength.toFixed(3)}
          "
          stroke-dashoffset="
            ${offset.toFixed(3)}
          "
  ...
  const totalVacation =
    Number(rest.allowance || 0) +
    Number(rest.carryover || 0);
  ...
  return `
    <article
      ...
      <div class="kpi-vacation">

        <div class="kpi-vacation-head">
          <span>
            Resturlaub ${vacationYear}
          </span>
        ...
        <div class="kpi-vacation-track">
          <i
            style="
              --vacation-progress:
              ${vacationProgress.toFixed(2)}%;
            ">
          </i>
        </div>
...
function getEmployeeExitDate(employee) {
  if (!employee) return null;

  // Bei aktuell aktiven Mitarbeitern wird kein Austritt berücksichtigt.
  if (employee.active !== false) return null;

  if (!employee.exitDate) return null;

  const exitDate = parseDateKey(employee.exitDate);

  return Number.isNaN(exitDate.getTime())
    ? null
    : exitDate;
}

function hasEmployeeStartedOnDate(employee, day) {
  if (!employee || !day) return false;
...
function isEmployeeEmployedOnDate(employee, day) {
  if (!employee || !day) return false;

  if (!hasEmployeeStartedOnDate(employee, day)) {
    return false;
  }

  const exitDate = getEmployeeExitDate(employee);

  // Inaktiv ohne Austrittsdatum:
  // nicht mehr als beschäftigt behandeln.
  if (employee.active === false && !exitDate) {
    return false;
  }

  if (exitDate && dateOnly(day) > dateOnly(exitDate)) {
    return false;
  }
...
function isEmployeeRelevantForPeriod(employee, view) {
  if (!employee || !view?.start || !view?.end) {
    return false;
  }

  // Alte inaktive Datensätze ohne Austrittsdatum
  // weiterhin aus der Abrechnung heraushalten.
  if (
    employee.active === false &&
    !getEmployeeExitDate(employee)
  ) {
  ...
  return getDaysInRange(view.start, view.end).some((day) =>
    isEmployeeEmployedOnDate(employee, day)
  );
}

function isEmployeeVisibleInAttendanceMonth(
  employee,
  view
) {
  if (
  ...
  /*
   * Eintritt erst nach diesem Monat:
   * Die Zeile wird noch nicht angezeigt.
   */
  if (
    entryDate &&
    dateOnly(entryDate) >
  ...
  /*
   * Alte inaktive Datensätze ohne Datum
   * bleiben weiterhin ausgeblendet.
   */
  if (
    employee.active === false &&
    !exitDate
  ...
  /*
   * Der Monat, in dem jemand inaktiv wird,
   * bleibt vollständig sichtbar.
   *
   * Erst ab dem folgenden Monat
   * verschwindet die Mitarbeiterzeile.
   */
  if (
    exitDate &&
    dateOnly(exitDate) <
...
function isEmployeeAttendanceEditableOnDate(
  employee,
  day
) {
  if (!employee || !day) {
  ...
  /*
   * Das gespeicherte Datum bedeutet:
   * "inaktiv ab diesem Tag".
   */
  if (
    exitDate &&
    dateOnly(day) >=
 ...
 counters[emp.name].hours += day.getDay() === 6 ? 4 : 9;
  ...
  function normalizeAttendanceReadEntry(entry) {
...
function getStoredAttendanceEntry(employeeId, key) {
  const entry = state.attendance?.[employeeId]?.[key];

  if (!entry || typeof entry !== "object") {
    return null;
  }

  return normalizeAttendanceReadEntry(entry);
}

function getManagementAttendanceEntry(bossId, key) {
  return normalizeAttendanceReadEntry(state.managementAttendance?.[bossId]?.[key]);
}

function getAttendanceEntry(employeeId, key) {
  const raw = getStoredAttendanceEntry(employeeId, key);

  if (raw) return raw;

  return getExpandedVacationEntry(employeeId, key)
    || { status: "", note: "" };
}

function getRawAttendanceEntry(employeeId, key) {
  return getStoredAttendanceEntry(employeeId, key)
    || { status: "", note: "" };
}
  ...
  function normalizeEventRecurrence(value) {
    const allowed = ["", "28d", "monthly", "quarterly", "yearly"];
    const text = String(value || "").trim();
    if (allowed.includes(text)) return text;
    if (text === "alle 28 Tage") return "28d";
    if (text === "monatlich") return "monthly";
    if (text === "alle 3 Monate") return "quarterly";
    if (text === "j\u00e4hrlich") return "yearly";
    return "";
  }

  function getEventRecurrenceLabel(value) {
    const recurrence = normalizeEventRecurrence(value);
    if (recurrence === "28d") return "alle 28 Tage";
    if (recurrence === "monthly") return "monatlich";
    if (recurrence === "quarterly") return "alle 3 Monate";
  ...
  function getEventOccurrenceDate(event, index) {
    if (!event.date) return null;
    const base = parseDateKey(event.date);
    if (Number.isNaN(base.getTime())) return null;

    const recurrence = normalizeEventRecurrence(event.recurrence);
 if (!recurrence || index === 0) return index === 0 ? base : null;

    if (recurrence === "28d") {
      const next = new Date(base);
      next.setDate(next.getDate() + index * 28);
      return next;
    }

    if (recurrence === "monthly") return addMonthsClamped(base, index);
    if (recurrence === "quarterly") return addMonthsClamped(base, index * 3);
    if (recurrence === "yearly") return addYearsClamped(base, index);

    return null;
  }

  function getEventOccurrencesInRange(event, fromDate, toDate) {
    const recurrence = normalizeEventRecurrence(event.recurrence);
    const from = parseDateKey(dateKey(fromDate));
    const to = parseDateKey(dateKey(toDate));
    const occurrences = [];

    if (!event.date) return occurrences;

    if (!recurrence) {
      const single = parseDateKey(event.date);
      if (!Number.isNaN(single.getTime()) && single >= from && single <= to) {
        occurrences.push({ ...event, date: dateKey(single), originalDate: event.date, recurrence });
      }
      return occurrences;
    }

    for (let i = 0; i < 500; i += 1) {
      const occurrenceDate = getEventOccurrenceDate(event, i);
      if (!occurrenceDate || Number.isNaN(occurrenceDate.getTime())) break;
      ...
      if (occurrenceDate >= from) {
        occurrences.push({
          ...
          recurrence,
          recurrenceLabel: getEventRecurrenceLabel(recurrence)
        });
      }
    }
  ...
  function getNextEventOccurrences(limit, fromDate = new Date(), maxDaysAhead = 366) {
    const from = parseDateKey(dateKey(fromDate));
    const to = new Date(from);
    to.setDate(to.getDate() + maxDaysAhead);

    return (state.events || [])
      .flatMap((event) => getEventOccurrencesInRange(event, from, to))
      .sort((a, b) =>
  ...
  function getUpcomingEvents(limit) {
    return getNextEventOccurrences(limit);
  }
...
function saveLocalStateCache() {
  try {
    localStorage.setItem(STORAGE_KEY, JSON.stringify(state));
  } catch (error) {
    console.warn(
      "Lokaler Cache konnte nicht gespeichert werden. Supabase-Speichern läuft weiter.",
      error
    );
  }
}
    ...
    return employees
      ...
      .map((employee) => {
        ...
        return {
          ...
          personnelNumber: String(employee.personnelNumber || ""),
  ...
  function normalizeEvents(events) {
    ...
    return events
      ...
      .map((item) => ({
        ...
        recurrence: normalizeEventRecurrence(item.recurrence || ""),
...
function getDashboardEventPreviewItems(limit = Infinity) {
  ...
  return getUpcomingEvents(20)
    ...
    .map((event) => ({
      ...
      meta: [
 event.date ? formatDate(parseDateKey(event.date)) : "Ohne Datum",
 event.recurrence ? getEventRecurrenceLabel(event.recurrence) : ""
      ].filter(Boolean).join(" · ")
    }))
    .sort((a, b) => a.sortKey.localeCompare(b.sortKey))
    .slice(0, limit);
