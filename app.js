(function () {
  ...
  const APP_CHANGELOG = [
    ...
    "REGU-Historie: Bulk-Import für mehrere Preislisten.",
    "REGU-Historie: Zusatzpreise separat erfasst.",
    "Fahrzeuge: FIN-Feld repariert.",
  ...
  const rows = state.employees
    ...
    .map((employee) => {
      ...
      return {
        ...
        sickHoliday: [sickText, holidayText].filter(Boolean).join(", ")
      };
    });
  ...
  printWindow.document.write(`
    ...
    <html lang="de">
      ...
      <body>
        ...
        <p>
          Die Journale sollen bitte vorab an Renaldo Guttenberger geschickt werden.
        </p>
...
if (addressInput && addressInput.value !== (state.settings.taxAdvisorAddress || "")) {
  ...
  target.innerHTML = rows.map((row) => {
    ...
    return `
      <div class="tax-hours-row" data-tax-employee="${escapeHtmlAttr(row.employeeId)}">
...
function calculateTaxHoursEntry(entry) {
  ...
  const vacationMinutes = vacationDays * 9 * 60;
  const holidayMinutes = holidayDays * 9 * 60;
  const sickMinutes = sickDays * 9 * 60;
  const totalMinutes = roundedMinutes + vacationMinutes + holidayMinutes + sickMinutes;
  ...
  return {
    ...
    totalLabel: formatMinutesAsDecimalHours(totalMinutes)
  };
}
...
function buildTaxHoursOutput(rows, view) {
  ...
  result.totalLabel,
  vacationText,
  [sickText, holidayText].filter(Boolean).join(" ")
].join("\t");
  }).join("\n");
}
...
$("#ownPriceHistoryResetRangeBtn")?.addEventListener("click", () => {
  ...
  $("#supplierHistoryMaterialSelect")?.addEventListener("change", renderSupplierHistoryChart);
  $("#supplierCompareMaterialSelect")?.addEventListener("change", renderSupplierCompareResults);
}
...
syncOwnPriceHistoryRangeInputs();
renderOwnPurchaseHistoryChart();
renderSupplierHistoryMaterialSelect();
renderSupplierHistoryChart();
renderSupplierCompareMaterialSelect();
renderSupplierCompareResults();
renderCurrentPriceResults();
}
...
function importOwnPurchasePriceExcel(event) {
  ...
  reader.onload = () => {
    try {
      ...
      const importDate = state.ownPurchasePrices.date || dateKey(new Date());
...
showToast(`${entries.length} eigene Ankaufspreise importiert und in der Historie gespeichert.`, "success");
            ...
            ? supplierMatches.slice(0, 16).map(({ list, entry }) => `
              <button
                ...
                <div>
                  <strong>${escapeHtml(entry.material)}</strong>
                  <small>
  ${escapeHtml(list.company || "")}
  · ${list.date ? formatDate(parseDateKey(list.date)) : "ohne Datum"}
  ${entry.reguArticleNumber || entry.reguMapping ? " · REGU " + escapeHtml(entry.reguArticleNumber || entry.reguMapping) : ""}
</small>
                </div>
    ...
    const cards = [
      {
        ...
        detail: priceStatus.needsAttention
          ? "Neue Preise bestätigen oder mit heutigem Datum aktualisieren."
