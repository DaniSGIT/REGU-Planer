(function () {
  ...
  $("#priceListPdfInput")?.addEventListener("change", importPriceListPdf);
...
function savePriceListDraft() {
  ...
  if (!state.priceList.pdfPath && !state.priceList.pdfData) {
    showToast("Bitte PDF hochladen.", "error");
    return;
  }
  ...
  state.priceLists.push({
    ...
    pdfPath: state.priceList.pdfPath || "",
    pdfData: state.priceList.pdfData || "",
...
function renderPriceListsList() {
      ...
      <div class="price-list-card" data-price-list-id="${escapeHtmlAttr(list.id)}">
        <div class="price-list-card-head">
          <div>
            ...
            <small>${list.date ? formatDate(parseDateKey(list.date)) : "Ohne Datum"} · ${escapeHtml(list.pdfName || "Keine PDF")}</small>
          </div>
          <div class="price-list-card-actions">
            <button class="ghost" type="button" data-open-price-pdf="${escapeHtmlAttr(list.id)}">PDF öffnen</button>
            <button class="danger" type="button" data-delete-price-list="${escapeHtmlAttr(list.id)}">Löschen</button>
          </div>
        </div>
  ...
  target.querySelectorAll("[data-delete-price-list]").forEach((button) =>
    button.addEventListener("click", () => {
      ...
      showConfirm(`Preisliste von "${list.company}" wirklich löschen?`, () => {
...
function removeBulkStatusRange() {
  ...
  async function importPriceListPdf(event) {
  const file = event.target.files?.[0];
  if (!file) return;
  ...
  showToast("PDF wird hochgeladen...", "success");
  ...
  if (error) {
    console.error(error);
    showToast("PDF konnte nicht hochgeladen werden.", "error");
    event.target.value = "";
    return;
  }

  state.priceList.pdfName = file.name;
  state.priceList.pdfPath = filePath;
  state.priceList.pdfData = "";

  try {
    showToast("PDF wird ausgelesen...", "success");

    const parsed = await parsePriceListPdfFile(file);

    if (parsed.company && !state.priceList.company) {
      state.priceList.company = parsed.company;
    }

    if (parsed.date && !state.priceList.date) {
      state.priceList.date = parsed.date;
    }

    if (parsed.entries.length) {
      state.priceList.entries = parsed.entries;
      showToast(`${parsed.entries.length} Preise aus PDF erkannt. Bitte kurz prüfen.`, "success");
    } else {
      showToast("PDF wurde hochgeladen, aber keine Preise erkannt.", "error");
    }
  } catch (err) {
    console.error(err);
    showToast("PDF wurde hochgeladen, aber nicht ausgelesen.", "error");
  }
  ...
  if (!list?.pdfPath && !list?.pdfData) {
    showToast("Keine PDF hinterlegt.", "error");
    return;
  }

  let pdfUrl = list.pdfData || "";

  if (list.pdfPath) {
    const { data, error } = await supabaseClient.storage
      .from(PRICE_LIST_BUCKET)
      .createSignedUrl(list.pdfPath, 60 * 10);
  ...
  document.body.appendChild(overlay);
  overlay.querySelector("[data-close-pdf]")?.addEventListener("click", () => overlay.remove());
}
...
async function saveOfficePlanToSupabase() {
  try {
    const payload = {
      ...
      // Einstellungen, die direkt beeinflussen, wie der Büroplan gelesen wird
      ...
      // Mini-Namensliste nur für Kalender/Anzeige, keine Personaldaten
...
function normalizeCalendarSlug(value) {
  ...
  function normalizePriceList(priceList) {
    return {
      company: String(priceList.company || ""),
      date: String(priceList.date || ""),
      material: String(priceList.material || ""),
      price: String(priceList.price || ""),
      pdfName: String(priceList.pdfName || ""),
      pdfData: String(priceList.pdfData || "")
    };
  }
