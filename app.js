(function () {
  ...
  const APP_CHANGELOG = [
    "Lieferantenlisten entfernt; REGU-Ankaufspreise und REGU-Historie bleiben erhalten.",
    "Neue Preise im Dashboard Hover",
    "Remote-Speicherungsupdate.",
    "Stundenabrechnung direkt bearbeitbar und exportierbar.",
    "Yesim-Stunden: Feiertage und Urlaub wird automatisch in Tatsächliche Stunden erfasst.",
    "Hofbuch: Suche, Badges und bearbeitbare Einträge."
  ];
...
function getTaxHoursAutoExtras(employeeId, view) {
  ...
  getDaysInRange(view.start, view.end).forEach((day) => {
    ...
    // Feiertage:
    // Normale Mitarbeiter bekommen Feiertage automatisch.
    // Yesim/Daniela bekommen Feiertage nur, wenn sie an dem Feiertag im Büroplan eingetragen sind.
    ...
    // Urlaub:
    // Nicht an Feiertagen/Sonntagen zählen.
    // Bei Yesim/Daniela gelten die Teilzeitregeln:
    // automatisch erweiterte Tage werden angezeigt, aber NICHT als Urlaubstag gezählt.
    if (
      attendance.status === "U" &&
      !isHoliday &&
      !isSunday &&
    ...
    // Fahrtgeld:
    // Nur echte Anwesenheit aus der Tageserfassung.
    // Kein Urlaub, kein Feiertag, kein Sonntag.
...
const result = calculateTaxHoursEntry(entry);
      ...
      const vacationText = result.vacationDays
        ? `${result.vacationDays} Tag${result.vacationDays === 1 ? "" : "e"}`
        : "";

      const sickText = result.sickDays
  ...
  <div class="tax-hours-row tax-hours-row-v2" data-tax-employee="${escapeHtmlAttr(row.employeeId)}">
    <div class="tax-hours-main">
      ...
      <label>
        ...
        <input type="number" min="0" step="1" value="${escapeHtmlAttr(entry.vacationDays)}" data-tax-field="vacationDays">
      </label>
...
function calculateTaxHoursEntry(entry) {
  ...
  const vacationDays = Number(entry.vacationDays || 0);
  const holidayDays = Number(entry.holidayDays || 0);
  const sickDays = Number(entry.sickDays || 0);
...
const result = calculateTaxHoursEntry(entry);
    ...
    const vacationText = result.vacationDays
      ? `${result.vacationDays} Urlaubstag${result.vacationDays === 1 ? "" : "e"}`
      : "";
...
function captureLocalViewState() {
  return {
    periodAnchor: structuredClone(state.settings.periodAnchor || {}),
    officeCounterAnchorDate: state.settings.officeCounterAnchorDate || "",
    dashboardAnchor: structuredClone(state.settings.dashboardAnchor || {}),
    statsAnchor: structuredClone(state.settings.statsAnchor || {}),
    vacationPlanYear: state.settings.vacationPlanYear || new Date().getFullYear(),
    vacationPlanBossId: state.settings.vacationPlanBossId || "",
    attendanceDay: state.settings.attendanceDay || dateKey(new Date()),
    focusedEmployeeId: state.settings.focusedEmployeeId || "",
    dashboardYear: state.settings.dashboardYear || new Date().getFullYear(),
    vehicleTab: state.settings.vehicleTab || "pkw",
    hofbookDate: state.settings.hofbookDate || dateKey(new Date())
  };
}
...
function bindHofbook() {
    $("#hofbookPrevBtn")?.addEventListener("click", () => shiftHofbookDay(-1));
  $("#hofbookTodayBtn")?.addEventListener("click", () => {
    state.settings.hofbookDate = dateKey(new Date());
    saveState();
    renderHofbook();
  });
    $("#hofbookNextBtn")?.addEventListener("click", () => shiftHofbookDay(1));
...
function openHofbookEntryModal(editId = "") {
  ...
  if (typeInput) typeInput.value = entry?.type === "hint" ? "hint" : "container";
...
function saveHofbookModalEntry() {
  ...
  const type = $("#hofbookModalType")?.value === "hint" ? "hint" : "container";
  ...
  saveState({ remote: ["hofbook", "settings"] });
  closeHofbookEntryModal();
  renderHofbook();
  ...
  return (Array.isArray(entries) ? entries : [])
    ...
    .map((entry) => {
      ...
      return {
        ...
        type: rawType === "hint" ? "hint" : "container",
...
function shiftHofbookDay(offset) {
  ...
  if (paper) {
    paper.classList.remove("hofbook-page-next", "hofbook-page-prev");
    void paper.offsetWidth;
    paper.classList.add(direction === "next" ? "hofbook-page-next" : "hofbook-page-prev");
  }

  const current = parseDateKey(getHofbookDate());
  current.setDate(current.getDate() + offset);

  state.settings.hofbookDate = dateKey(current);
  saveState();
  ...
  setTimeout(() => {
    ...
    if (paper) {
      paper.classList.remove("hofbook-page-next", "hofbook-page-prev");
      paper.classList.add("hofbook-page-in");

      setTimeout(() => {
        paper.classList.remove("hofbook-page-in");
      }, 220);
    }
  }, 130);
}
...
const notes = (state.notes || []).filter((note) => note.date === currentKey);
  ...
  if (title) {
    ...
    title.innerHTML = `
      ...
      ${
        notes.length
          ? `<button class="hofbook-day-badge note" type="button" data-hofbook-open-notes>📝 ${notes.length} Notiz${notes.length === 1 ? "" : "en"}</button>`
          : ""
      }
    `;
  }
...
function getHofbookBossAbsenceHints(dayKey) {
  ...
  (state.hofbookEntries || []).forEach((entry) => {
    ...
    if (text.includes("rg nicht da")) labels.add("RG nicht da!");
    if (text.includes("tg nicht da")) labels.add("TG nicht da!");
  });
...
function renderHofbookPaper() {
  ...
  const entries = (state.hofbookEntries || [])
    .filter((entry) => entry.date === currentKey)
    .sort(sortHofbookEntries);

  const hints = entries.filter((entry) => {
  if (entry.type !== "hint") return false;

  const text = normalizeHofbookSearchText(entry.text || "");

  if (text.includes("rg nicht da")) return false;
  if (text.includes("tg nicht da")) return false;

  return true;
});
  const orders = entries.filter((entry) => entry.type !== "hint");

  if (!entries.length) {
    paper.innerHTML = `
      <div class="hofbook-paper-empty">
  ...
  paper.innerHTML = `
    ${
      hints.length
        ? `
          <div class="hofbook-paper-hints">
            ${hints.map((entry) => buildHofbookHintPill(entry)).join("")}
          </div>
        `
        : ""
    }

    <div class="hofbook-paper-lines">
      ${
        orders.length
          ? orders.map((entry) => buildHofbookOrderLine(entry)).join("")
          : `<div class="hofbook-paper-empty small">Keine Bestellungen eingetragen.</div>`
      }
    </div>
  `;
}
...
function renderHofbookSearch() {
  ...
  const ordersCount = matches.filter((entry) => entry.type !== "hint").length;
  const doneCount = matches.filter((entry) => entry.done).length;
  const openCount = matches.filter((entry) => !entry.done && entry.type !== "hint").length;
  ...
  results.innerHTML = matches
    ...
    .map((entry) => {
      ...
      const typeLabel = entry.type === "hint" ? "Hinweis" : "Bestellung";
      const statusLabel = entry.type === "hint" ? "" : entry.done ? " · erledigt" : " · offen";
...
function collectHofbookSearchResults() {
  ...
  return (state.hofbookEntries || [])
    .filter((entry) => {
      ...
      if (filter === "container" && entry.type === "hint") return false;
      if (filter === "hint" && entry.type !== "hint") return false;
      if (filter === "open" && (entry.type === "hint" || entry.done)) return false;
      if (filter === "done" && !entry.done) return false;
...
function buildHofbookSearchHaystack(entry) {
  ...
  const dateText = [
    ...
    entry.type === "hint" ? "hinweis" : "bestellung",
...
function sortHofbookEntries(a, b) {
  const typeCompare = (a.type === "hint" ? 0 : 1) - (b.type === "hint" ? 0 : 1);
  ...
  rows.forEach((row) => {
    ...
    for (let i = 0; i < values.length; i += 1) {
      ...
      entries.forEach((entry) => {
        const key = `${entry.articleNumber}_${normalizeMaterialText(entry.material)}_${entry.priceKg}`;
        if (seen.has(key)) return;
        seen.add(key);
        result.push(entry);
      });
...
function buildOwnPriceEntriesFromMaterialText({ articleNumber, rawMaterial, priceKg, isRequestPrice }) {
...
function renderOwnPurchaseHistoryMaterialSelect() {
  ...
  (state.ownPurchasePriceHistory || []).forEach((list) => {
    (list.entries || []).forEach((entry) => {
      ...
      const label = String(entry.material || "").trim();
      if (!label) return;
...
function collectOwnPriceHistoryChartPoints(selectedKey) {
  ...
  (state.ownPurchasePriceHistory || []).forEach((list) => {
    ...
    const matches = (list.entries || []).filter((entry) => {
      if (entry.onRequest) return false;
      if (!Number(entry.priceKg || 0)) return false;

      const materialKey = normalizeMaterialText(entry.material || "");
...
return materialKey === selectedKey;
    ...
    // Wenn mehrere Positionen in derselben Gruppe sind, nehmen wir den höchsten Preis.
    // Später können wir das feiner machen: exakte Sorte statt Gruppe.
    const bestEntry = [...matches].sort((a, b) => Number(b.priceKg || 0) - Number(a.priceKg || 0))[0];

    bestByDate.set(listDate, {
  date: listDate,
  label: bestEntry.materialGroup || bestEntry.material || "Material",
  material: bestEntry.material || "",
  priceKg: Number(bestEntry.priceKg || 0),
  excelName: list.excelName || "",
  listId: list.id || ""
});
  });
    ...
    ? `
      <article class="price-current-card own selected-regu-price">
        ...
        <div class="price-selected-box">
          ...
          <b>${selectedOwn.onRequest ? "auf Anfrage" : formatEuroPerKg(selectedOwn.priceKg)}</b>
        </div>
  ...
  function renderVacationPlanner() {
    ...
    const people = [
      ...
      ...state.externalBirthdays
        .map((boss) => ({ id: boss.id, name: boss.name || "Geschäftsfuehrung", type: "boss" }))
    ];
...
function getReguPriceComparableValue(entry) {
  if (entry?.onRequest) return "auf-anfrage";

  const priceKg = Number(entry?.priceKg || 0);

  return Number.isFinite(priceKg) ? priceKg.toFixed(4) : "0.0000";
}
...
async function saveModuleToSupabase(moduleName) {
  if (moduleName === "all") {
    await saveAllModulesToSupabase();
    return;
  }
  ...
  try {
    ...
    // Die alte bueroplan-Tabelle bleibt für den abonnierbaren Kalender erhalten.
