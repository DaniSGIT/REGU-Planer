...
function getTaxHoursAutoExtras(employeeId, view) {
  ...
  rangeDays.forEach((day) => {
    ...
    if (
      ...
      vacationDays += getVacationDayValue(attendance);
    }
...
function renderHofbookPaper() {
  ...
  const hints = entries.filter((entry) => entry.type === "hint");
...
function getVacationDayValue(entry) {
  if (!entry || entry.status !== "U") return 0;
  return normalizeVacationValue(entry.vacationValue ?? 1);
}
        ...
        return `
          <div class="daily-row ${entry.status === "S" ? "show-note" : ""} ${isNonCountedVacationDay ? "non-counted-vacation-attendance" : ""} ${isSundayDay ? "sunday-attendance" : ""} ${holidayName ? "holiday-attendance" : ""} ${isClosedDay ? "closed-attendance" : ""}" style="${nonCountedStyle}" data-employee-id="${emp.id}">
            ...
            <div class="daily-controls">
              ...
              <label class="daily-vacation-value ${entry.status === "U" ? "" : "hidden"}">
  <span>Urlaubsanteil</span>
  <select
    data-vacation-value
    data-employee="${emp.id}"
    ${entry.status === "U" && !isBlockedDay ? "" : "disabled"}>
  ...
  function buildVacationBarsForMonth(people, days) {
    ...
    people.forEach((person) => {
      ...
      days.forEach((day, index) => {
        const key = dateKey(day);
        const status = person.type === "boss"
  ? (getManagementPlannerEntry(person, key)?.status || "")
  : (getAttendanceEntry(person.id, key)?.status || "");
...
function getManagementPlannerEntry(person, key) {
  ...
  if (!status) {
    ...
    const old = getRawAttendanceEntry(employeeId, date) || { status, note: "", vacationValue: 1 };
    ...
    if (status === "U") {
      nextEntry.vacationValue = old.status === "U"
        ? normalizeVacationValue(old.vacationValue)
        : 1;
    }
...
function onVacationValueChange(e) {
  ...
  if (!old || old.status !== "U") return;

  setAttendanceEntry(employeeId, date, {
    ...old,
    vacationValue: normalizeVacationValue(e.target.value)
  });
